...
function amdgpu_moe_forward!(moe::AMDGPUMoELayer, x::ROCArray{Float32, 2}; 
                            training::Bool = false)
    ...
    amdgpu_router_forward!(moe.router, x, router_logits, router_probs, 
                          expert_indices, expert_gates; training=training)
    
    output = AMDGPU.zeros(Float32, config.output_dim, batch_size)
    amdgpu_process_experts!(output, moe.experts, x, expert_indices, 
                           expert_gates, config)
    
    balance_loss = 0.0f0
    if training
        balance_loss = amdgpu_compute_balance_loss(expert_indices, router_probs,
