...
function amdgpu_router_forward!(router::AMDGPURouter, x::ROCArray{Float32, 2}, 
                               router_logits::ROCArray{Float32, 2},
                               router_probs::ROCArray{Float32, 2},
    ...
    if training && router.noise_scale > 0.0f0
        if !isnothing(router.noise_weight)
            ...
            noise = AMDGPU.randn(Float32, size(router_logits)) .* 
                   NNlib.softplus.(noise_logits) .* router.noise_scale
            router_logits .+= noise
        else
            noise = AMDGPU.randn(Float32, size(router_logits)) .* router.noise_scale
            router_logits .+= noise
        end
    end
    ...
    try
        ...
        @roc groupsize=threads_per_block gridsize=blocks topk_kernel!(
            expert_indices, expert_gates, router_probs, router.k, num_experts, batch_size)
        AMDGPU.synchronize()
    catch e
        @warn "AMDGPU TopK kernel failed, falling back to CPU: $e"
        ...
        expert_indices .= ROCArray(expert_indices_cpu)
        expert_gates .= ROCArray(expert_gates_cpu)
    end
    
    return router_logits, router_probs, expert_indices, expert_gates
end

function amdgpu_compute_balance_loss(expert_indices::ROCArray{Int32, 2},
                                    router_probs::ROCArray{Float32, 2},
                                    expert_counts::ROCArray{Float32, 1},
    ...
    @roc groupsize=threads_per_block gridsize=blocks count_assignments_kernel!(
